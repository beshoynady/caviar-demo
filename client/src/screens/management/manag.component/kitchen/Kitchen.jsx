                      ...
                      <div className="card text-white bg-success" style={{ width: "265px" }}>
                        ...
                        <ul className='list-group list-group-flush'>
                          {order.products.filter((pr) => pr.isDone === false).map((product, i) => {
                            return (
                              <div key={i} className={`${product.isAdd ? 'bg-red' : 'bg-light'}`}>
                                <li className={`list-group-item text-dark d-flex justify-content-between align-items-center bg-transparent ${product.isAdd ? 'bg-red' : 'bg-light'}`}>
