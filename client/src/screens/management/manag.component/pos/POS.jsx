                        ...
                        {allProducts.filter(pro => pro.category === categoryid).map((product, index) => {
                          return (
                            <div className='col-md-3 mb-4' key={index}>
                              <div className="card rounded-3 border-0 shadow" style={{ width: '10rem', height: '15rem' }} onClick={() => additemtocart(product._id)}>
                                <img className='card-img-top' src={`https://raw.githubusercontent.com/beshoynady/restaurant-api/main/server/images/${product.image}`} alt="" style={{ height: '7rem', objectFit: 'cover' }} />
                                <div className="card-body bg-dark text-white d-flex flex-column justify-content-between">
                                  <div>
                                    <h6 className='card-title text-center mb-2 font-weight-bold'>{product.name}</h6>
                                    <p className='card-text text-center mb-2 font-weight-bold'>{product.priceAfterDiscount ? product.priceAfterDiscount : product.price}ج</p>
                                  </div>
